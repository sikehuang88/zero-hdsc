...
class TestAppraisalResult:
    ...
    def test_rejects_out_of_range_0_1(self):
        with pytest.raises(ValueError):
            AppraisalResult(
                novelty=1.5, goal_congruence=0, controllability=0.5,
                certainty=0.5, self_agency=0.5, user_agency=0.5,
                external_agency=0.5, relationship_relevance=0.5,
                urgency=0.5, valence_signal=0, arousal_signal=0.5,
            )

    def test_rejects_out_of_range_neg1_1(self):
        with pytest.raises(ValueError):
            AppraisalResult(
                novelty=0.5, goal_congruence=2.0, controllability=0.5,
                certainty=0.5, self_agency=0.5, user_agency=0.5,
                external_agency=0.5, relationship_relevance=0.5,
                urgency=0.5, valence_signal=0, arousal_signal=0.5,
            )
    ...
    def test_accepts_valid_ranges(self):
        a = AppraisalResult(
            novelty=1.0, goal_congruence=-1.0, controllability=0.0,
            certainty=1.0, self_agency=0.5, user_agency=0.5,
            external_agency=0.5, relationship_relevance=1.0,
            urgency=0.0, valence_signal=1.0, arousal_signal=0.0,
        )
...
def make_event(content: str = "hello", event_id: str = "evt-1") -> Event:
    return Event(
        id=event_id, correlation_id="corr-1", conversation_id="c1",
        actor=Actor.USER, event_type="user.message",
        source_kind=SourceKind.USER_OBSERVED, content=content,
        content_hash="abc", created_at_ms=1000,
    )
...
class TestOrganismState:
    ...
    def test_rejects_out_of_range(self):
        with pytest.raises(ValueError):
            OrganismState(
                version=1, energy=2.0, connection_need=0.5,
                autonomy_need=0.5, curiosity=0.5, safety=0.5,
                valence=0.0, arousal=0.5, updated_at_ms=1000,
            )

    def test_rejects_valence_out_of_range(self):
        with pytest.raises(ValueError):
            OrganismState(
                version=1, energy=0.5, connection_need=0.5,
                autonomy_need=0.5, curiosity=0.5, safety=0.5,
                valence=2.0, arousal=0.5, updated_at_ms=1000,
            )
...
class TestDeterministicStateEngine:
    ...
    def test_preview_returns_new_state(self, engine, initial_state):
        appraisal = AppraisalResult(
            novelty=0.8, goal_congruence=0.5, controllability=0.7,
            certainty=0.9, self_agency=0.3, user_agency=0.7,
            external_agency=0.2, relationship_relevance=0.6,
            urgency=0.3, valence_signal=0.7, arousal_signal=0.5,
        )
    ...
    def test_preview_is_deterministic(self, engine, initial_state):
        appraisal = AppraisalResult(
            novelty=0.6, goal_congruence=0.3, controllability=0.5,
            certainty=0.7, self_agency=0.3, user_agency=0.5,
            external_agency=0.3, relationship_relevance=0.4,
            urgency=0.4, valence_signal=0.2, arousal_signal=0.4,
        )
    ...
    def test_state_always_in_bounds(self, engine, initial_state):
        ...
        for _ in range(100):
            appraisal = AppraisalResult(
                novelty=random.random(), goal_congruence=random.uniform(-1, 1),
                controllability=random.random(), certainty=random.random(),
                self_agency=random.random(), user_agency=random.random(),
                external_agency=random.random(), relationship_relevance=random.random(),
                urgency=random.random(), valence_signal=random.uniform(-1, 1),
    ...
    def test_energy_recovers_over_time(self, engine):
        ...
        low_energy = OrganismState(
            version=1, energy=0.2, connection_need=0.5,
            autonomy_need=0.5, curiosity=0.5, safety=0.5,
            valence=0.0, arousal=0.2, updated_at_ms=1000,
        )
        # 10 hours later.
        preview = engine.preview(low_energy, AppraisalResult.neutral(), now_ms=1000 + 10 * 3600 * 1000)
    ...
    def test_valence_ema(self, engine, initial_state):
        ...
        positive = AppraisalResult(
            novelty=0.5, goal_congruence=0.5, controllability=0.5,
            certainty=0.5, self_agency=0.3, user_agency=0.5,
            external_agency=0.3, relationship_relevance=0.5,
            urgency=0.3, valence_signal=1.0, arousal_signal=0.3,
        )
